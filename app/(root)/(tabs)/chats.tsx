...
const chats = () => {
    ...
    const filteredChats = searchText.trim() === '' ? userChats : userChats.filter((chat: Chat) => {
        return chat.users.some((other_user) => 
            other_user._id !== user?.id &&
            (other_user.username.toLowerCase().includes(searchText.toLowerCase()) ||
            other_user.name.toLowerCase().includes(searchText.toLowerCase()))
        );
    });
